...
#include "System/UI/Imgui/ImguiExtensions.h"
...
namespace KRG
{
    namespace ImGuiX
    {
        ...
        bool RotationGizmo::DrawRotationWidget( Render::Viewport const& viewport, Vector const& axisOfRotation_WS, Color color )
        {
            ...
            pDrawList->AddPolyline( circlePointsSS, g_halfCircleSegmentCount, ImGuiExtensions::GetImguiColorU32( color ), false, 3.0f );
            return isHovered;
        }
        ...
        void RotationGizmo::DrawManipulationWidget( Render::Viewport const& viewport, Vector const& axisOfRotation_ws, Vector const& axisOfRotation_ss, Color color )
        {
            ...
            auto axisOfRotationColor = ImGuiExtensions::GetImguiColorU32( color );
            auto axisOfRotationColorAlpha = ImGuiExtensions::GetImguiColorU32( color.GetAlphaVersion( 0.75f ) );
            ...
            if ( Math::Abs( m_rotationDeltaAngle ) < Degrees( 3.0f ) )
            {
                ...
                pDrawList->AddCircleFilled( m_origin_SS, 3.0f, ImGuiExtensions::GetImguiColorU32( Colors::White ) );
                pDrawList->AddLine( m_origin_SS, pointOnCircle_SS, ImGuiExtensions::GetImguiColorU32( Colors::White ), 1.0f );
                pDrawList->AddCircleFilled( pointOnCircle_SS, 3.0f, ImGuiExtensions::GetImguiColorU32( Colors::Orange ) );
            }
            else
            {
                ...
                pDrawList->AddConvexPolyFilled( innerCirclePointsSS, numCoveredCirclePoints + 1, ImGuiExtensions::GetImguiColorU32( Colors::Orange.GetAlphaVersion(0.5f) ) );
                pDrawList->AddPolyline( innerCirclePointsSS, numCoveredCirclePoints, ImGuiExtensions::GetImguiColorU32( Colors::Orange ), false, 4.0f );
                pDrawList->AddLine( m_origin_SS, innerCirclePointsSS[numCoveredCirclePoints - 1], ImGuiExtensions::GetImguiColorU32( Colors::Orange ), 1.0f );
                pDrawList->AddLine( m_origin_SS, innerCirclePointsSS[0], ImGuiExtensions::GetImguiColorU32( Colors::Orange ), 1.0f );
                pDrawList->AddCircleFilled( m_origin_SS, 3.0f, ImGuiExtensions::GetImguiColorU32( Colors::White ) );
                ...
                pDrawList->AddRectFilled( textPosition, textPosition + Float2( textSize.x + 3, textSize.y ), ImGuiExtensions::GetImguiColorU32( Colors::Black.GetAlphaVersion( 0.5f ) ), 3.0f );
                pDrawList->AddText( textPosition, ImGuiExtensions::GetImguiColorU32( Colors::White ), buff );
            }
        }
        ...
        void RotationGizmo::PerformManipulation( Render::Viewport const& viewport )
        {
            ...
            if ( io.MouseDownDuration[0] > 0 )
            {
                ...
                if ( m_mode == ManipulationMode::RotateScreen )
                {
                    ...
                    bool isRotationDirectionPositive = Vector::Cross3( originalScreenDir, desiredRotationScreenDir ).z > 0.0f;
