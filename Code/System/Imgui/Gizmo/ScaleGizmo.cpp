...
namespace KRG
{
    namespace ImGuiX
    {
        ...
        bool ScalingGizmo::DrawPlaneManipulationWidget( Vector const& origin, Vector const& axis0, Vector const& axis1, Color color )
        {
            ...
            pDrawList->AddPolyline( manipulationQuadVerts, 3, ImGuiExtensions::GetImguiColorU32( color ), true, 1.0f );
            pDrawList->AddConvexPolyFilled( manipulationQuadVerts, 3, ImGuiExtensions::GetImguiColorU32( color.GetAlphaVersion( 0.75f ) ) );
            ...
            if ( io.MouseDownDuration[0] > 0 )
            {
                ...
                if ( !mouseDelta.IsNearZero2() )
                {
                    ...
                    if ( m_mode == ManipulationMode::ScaleXYZ )
                    {
                        ...
                        scaleDelta = Vector::One * -mouseDelta.y * unitsPerPixel;
                    }
                    ...
                    else if ( m_mode == ManipulationMode::ScaleX || m_mode == ManipulationMode::ScaleY || m_mode == ManipulationMode::ScaleZ )
                    {
                        ...
                        Vector const axisDeltaDir = Vector::Dot2( axisDelta.GetNormalized2(), manipulationAxisLine.GetDirection() ).x > 0 ? Vector::One : Vector::NegativeOne;
        ...
        void ScalingGizmo::InternalUpdate( Render::Viewport const& viewport )
        {
            ...
            auto drawAxisGuide = [&pDrawList, this] ( Vector const& axis, Color color )
            {
                ...
                pDrawList->AddLine( lineStart.ToFloat2(), lineEnd.ToFloat2(), ImGuiExtensions::GetImguiColorU32( color ), 2.0f );
            };
