...
namespace KRG::ImGuiX
{
    ...
    LRESULT ImguiSystem::ProcessInput( PlatformInputData const& data )
    {
        ...
        {
            // NOTE: Re-enable when we have multiple window support
            ...
            case WM_SYSKEYUP:
            {
                bool const isKeyDown = ( data.m_message == WM_KEYDOWN || data.m_message == WM_SYSKEYDOWN );
                if ( data.m_wParam < 256 )
                {
                    io.KeysDown[data.m_wParam] = isKeyDown;
                }
                if ( data.m_wParam == VK_CONTROL )
                {
                    io.KeyCtrl = isKeyDown;
                }

                if ( data.m_wParam == VK_SHIFT )
                {
                    io.KeyShift = isKeyDown;
                }

                if ( data.m_wParam == VK_MENU )
                {
                    io.KeyAlt = isKeyDown;
                }
            }
            ...
            case WM_KILLFOCUS:
            {
                io.AddFocusEvent( data.m_message == WM_SETFOCUS );
            }
            break;
