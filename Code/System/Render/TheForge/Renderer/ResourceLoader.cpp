...
#include "../Core/TextureContainers.h"
...
static UploadFunctionResult loadGeometry(Renderer* pRenderer, CopyEngine* pCopyEngine, size_t activeSet, UpdateRequest& pGeometryLoad)
{
	...
	if (iext[0] != 0 && (stricmp(iext, "gltf") == 0 || stricmp(iext, "glb") == 0))
	{
		...
		const bool structuredBuffers = (pDesc->mFlags & GEOMETRY_LOAD_FLAG_STRUCTURED_BUFFERS) != 0;
		...
		switch (gSelectedRendererApi)
		{
			...
			case RENDERER_API_D3D12:
				d3d12_compileShader(
					...
					(loadDesc.mFlags & SHADER_STAGE_LOAD_FLAG_ENABLE_PS_PRIMITIVEID) != 0, macroCount, pMacros, pOut, loadDesc.pEntryPointName);
