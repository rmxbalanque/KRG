...
#if _WIN32
#pragma warning( disable: 4389 4996 4245 4389 4456 4244 4800 4324 4005 4702 )
#endif
...
#endif
#include "../../ThirdParty/D3D12MemoryAllocator/Direct3D12MemoryAllocator.h"
	...
	if (cbvSrvUavDescCount || samplerDescCount)
	{
		if (cbvSrvUavDescCount)
		{
			...
			for (uint32_t i = 0; i < pRootSignature->mDescriptorCount; ++i)
			{
				...
				if (!pDescInfo->mRootDescriptor && pDescInfo->mType != DESCRIPTOR_TYPE_SAMPLER && pDescInfo->mUpdateFrequency == (uint32_t)updateFreq)
				{
...
void d3d12_queuePresent(Queue* pQueue, const QueuePresentDesc* pDesc)
{
	if (pDesc->pSwapChain)
	{
		...
		if (FAILED(hr))
		{
			...
			if (FAILED(removeHr))
			{
				LOGF(LogLevel::eERROR, "Device was removed 0x%08x", removeHr);
			...
			device->Release();
#endif
			LOGF(LogLevel::eERROR, "Failed to present swapchain render target 0x%08x", hr);
		}
	}
}
...
uint32_t d3d12_cmdWriteMarker(Cmd* pCmd, MarkerType markerType, uint32_t markerValue, Buffer* pBuffer, size_t offset, bool useAutoFlags)
{
	...
	uint32_t                             count = MARKER_TYPE_IN_OUT == markerType ? 2 : 1;
...
{
	uint32_t* pAlivePageCount;
	uint32_t* pRemovePageCount;
	uint32_t* pAlivePages;
	uint32_t* pRemovePages;
	uint32_t mTotalSize;
};
static VTReadbackBufOffsets vtGetReadbackBufOffsets(uint32_t* buffer, uint32_t readbackBufSize, uint32_t pageCount, uint32_t currentImage)
{
	...
	offsets.mTotalSize = (uint32_t)((offsets.pRemovePages - offsets.pAlivePageCount) + pageCount) * sizeof(uint32_t);
	return offsets;
}
...
void d3d12_releasePage(Cmd* pCmd, Texture* pTexture, uint32_t currentImage)
{
	...
	const uint32_t removePageCount = *readbackOffsets.pRemovePageCount;
	...
	uint32_t removePageIndex = 0;
	while (removePageIndex < removePageCount)
 	{
		const uint32_t MAX_UNBINDS = 1024;
	...
	eastl::hash_map<ID3D12Heap*, D3D12VTHeapPageData>& heapPageData, uint32_t currentImage)
{
	...
	if (allocateVirtualPage(pRenderer, pTexture, *pPage, &pIntermediateBuffer))
	{
		...
		startCoord.X = pPage->mD3D12.offset.X / (uint32_t)pTexture->pSvt->mSparseVirtualTexturePageWidth;
		startCoord.Y = pPage->mD3D12.offset.Y / (uint32_t)pTexture->pSvt->mSparseVirtualTexturePageHeight;
...
void d3d12_fillVirtualTexture(Cmd* pCmd, Texture* pTexture, Fence* pFence, uint32_t currentImage)
{
	...
	const uint32_t alivePageCount = *readbackOffsets.pAlivePageCount;
...
void d3d12_addVirtualTexture(Cmd* pCmd, const TextureDesc* pDesc, Texture** ppTexture, void* pImageData)
{
	...
	pTexture->mFormat = pDesc->mFormat;
	ASSERT(pTexture->mFormat == (uint32_t)pDesc->mFormat);
	...
	pTexture->pSvt->mPageVisibilityBufferSize = pTexture->pSvt->mVirtualPageTotalCount * 2 * sizeof(uint32_t);
			...
			for (uint32_t z = 0; z < sparseBindCounts.Z; z++)
			{
				for (uint32_t y = 0; y < sparseBindCounts.Y; y++)
				{
					for (uint32_t x = 0; x < sparseBindCounts.X; x++)
					{
						...
						VirtualTexturePage *newPage = addPage(pRenderer, pTexture, offset, extent, (uint32_t)pTexture->pSvt->mSparseVirtualTexturePageWidth * (uint32_t)pTexture->pSvt->mSparseVirtualTexturePageHeight * sizeof(uint32_t), mipLevel, layer, currentPageIndex);
						currentPageIndex++;
					}
				}
			}
