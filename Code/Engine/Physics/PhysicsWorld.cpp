...
namespace KRG
{
    namespace Physics
    {
        UpdatePriorityList const PhysicsWorld::PriorityList = UpdatePriorityList().EnableUpdateStage( UpdateStage::Physics );
        ...
        void PhysicsWorld::InitializeEntitySystem( SystemRegistry const& systemRegistry )
        {
            ...
            sceneDesc.cpuDispatcher = PhysX::Core->m_pDispatcher;
        ...
        void PhysicsWorld::ShutdownEntitySystem()
        {
            KRG_ASSERT( PhysX::Physics != nullptr );

            #if KRG_DEBUG_INSTRUMENTATION
            if ( PhysX::Core->m_pPVD->isConnected() )
            {
                PhysX::Core->m_pPVD->disconnect();
            }
            #endif
        ...
        void PhysicsWorld::UpdateEntitySystem( UpdateContext const& ctx )
        {
            {
        ...
        void PhysicsWorld::RegisterComponent( Entity const* pEntity, EntityComponent* pComponent )
        {
        ...
        void PhysicsWorld::UnregisterComponent( Entity const* pEntity, EntityComponent* pComponent )
        {
        ...
        {
            KRG_ASSERT( PhysX::Core->m_pPVD != nullptr );
            return PhysX::Core->m_pPVD->isConnected();
        }
        ...
        void PhysicsWorld::ConnectToPVD( Seconds timeToRecord )
        {
            ...
            if ( !PhysX::Core->m_pPVD->isConnected() )
            {
                PhysX::Core->m_pPVD->connect( *PhysX::Core->m_pPVDTransport, PxPvdInstrumentationFlag::eALL );
        ...
        void PhysicsWorld::UpdateRecordingPVD( Seconds TimeDelta )
        {
        ...
        void PhysicsWorld::ReflectDebugVisualizationState()
        {
            KRG_ASSERT( m_pScene != nullptr );
