...
namespace KRG
{
    namespace Animation
    {
        ...
        inline Transform const& Pose::GetTransform( int32 boneIdx ) const
        {
            KRG_ASSERT( boneIdx < GetNumBones() );
            return m_localTransforms[boneIdx];
        }

        inline void Pose::SetTransform( int32 boneIdx, Transform const& transform )
        { 
            KRG_ASSERT( boneIdx < GetNumBones() && boneIdx >= 0 ); 
            m_localTransforms[boneIdx] = transform;
        }

        inline void Pose::SetRotation( int32 boneIdx, Quaternion const& rotation )
        {
            KRG_ASSERT( boneIdx < GetNumBones() && boneIdx >= 0 );
            m_localTransforms[boneIdx].SetRotation( rotation );
        }

        inline void Pose::SetTranslation( int32 boneIdx, Float3 const& translation )
        {
            KRG_ASSERT( boneIdx < GetNumBones() && boneIdx >= 0 );
            m_localTransforms[boneIdx].SetTranslation( translation );
        }

        inline void Pose::SetScale( int32 boneIdx, Float3 const& scale )
        {
            KRG_ASSERT( boneIdx < GetNumBones() && boneIdx >= 0 );
