...
{
    BlendTask::BlendTask( int32 sourceNodeIdx, float const blendWeight, TBitFlags<Blender::PoseBlendOptions> blendOptions, BoneMask const* pBoneMask )
        : Task( sourceNodeIdx )
    ...
    void BlendTask::Execute( TaskPoseStack& poseStack ) const
    {
        ...
        if ( m_blendOptions.IsFlagSet( Blender::PoseBlendOptions::LockUnmaskedBones ) )
        {
