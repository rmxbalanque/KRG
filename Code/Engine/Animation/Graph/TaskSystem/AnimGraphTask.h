...
#include "AnimGraphTaskPosePool.h"
...
namespace KRG::Animation::Graph
{
    ...
    using TaskDependencies = TInlineVector<int8, 2>;
    ...
    public:
        ...
        Task( int16 sourceNodeIdx, UpdateStage updateStage = UpdateStage::Any, TaskDependencies const& dependencies = TaskDependencies() );
        virtual ~Task() {}
    ...
    protected:
        ...
        inline PoseBuffer* TransferDependencyPoseBuffer( TaskContext const& context, int8 dependencyIdx )
        {
            KRG_ASSERT( m_bufferIdx == InvalidIndex );
        ...
        inline PoseBuffer* AccessDependencyPoseBuffer( TaskContext const& context, int8 dependencyIdx )
        {
            auto pDependencyTask = context.m_dependencies[dependencyIdx];
            KRG_ASSERT( pDependencyTask != nullptr && pDependencyTask->m_isComplete && pDependencyTask->m_bufferIdx != InvalidIndex );
        ...
        inline void ConsumePoseBuffer( TaskContext const& context, int8 dependencyIdx )
        {
            auto pDependencyTask = context.m_dependencies[dependencyIdx];
            KRG_ASSERT( pDependencyTask != nullptr && pDependencyTask->m_isComplete && pDependencyTask->m_bufferIdx != InvalidIndex );
