...
namespace KRG::EntityModel
{
    ...
    {
        m_ID.Clear(); // Clear autogenerated ID
    }
    ...
    void EntityMap::AddEntityCollection( Transform const& collectionTransform, EntityCollection& entityCollection )
    {
        TVector<Entity*> entities;
        entityCollection.TransferEntities( entities );
        for ( auto& pEntity : entities )
        {
            // Shift collection to supplied position
            if ( pEntity->IsSpatialEntity() )
            {
                pEntity->SetWorldTransform( pEntity->GetWorldTransform() * collectionTransform );
            }
        ...
        else if ( m_pMapDesc.IsLoaded() )
        {
            if ( m_pMapDesc->IsValid() )
            {
                // Create entities
                {
                    ...
                    EntityCollectionDescriptor const& collectionDesc = *m_pMapDesc.GetPtr();
    ...
    void EntityMap::ProcessEntityAdditionAndRemoval( EntityLoadingContext const& loadingContext, EntityModel::ActivationContext& activationContext )
    {
        ...
        for ( int32 i = (int32) m_entitiesToRemove.size() - 1; i >= 0; i-- )
        {
            ...
            else // Remove from loading list as we might still be loaded this entity
            {
                ...
                EntityCollection::RemoveEntityFromCollection( pEntityToRemove->m_ID );
