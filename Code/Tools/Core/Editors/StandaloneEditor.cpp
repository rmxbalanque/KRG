...
namespace KRG
{
    ...
    void StandaloneEditor::Initialize( UpdateContext const& context, SettingsRegistry const& settingsRegistry )
    {
        KRG_ASSERT( m_pModel != nullptr );
        m_pModel->Initialize( context );
    }

    void StandaloneEditor::Shutdown( UpdateContext const& context )
    {
        KRG_ASSERT( m_pModel != nullptr );
        m_pModel->Shutdown( context );
    }

    void StandaloneEditor::DrawMainMenu( UpdateContext const& context, Render::ViewportManager& viewportManager )
    {
    ...
    void StandaloneEditor::DrawPopups( UpdateContext const& context )
    {
...
namespace KRG
{
    KRG_DEFINE_GLOBAL_REGISTRY( StandaloneEditorFactory );

    //-------------------------------------------------------------------------

    StandaloneEditor* StandaloneEditorFactory::TryCreateEditor( StringID const& editorModeID )
    {
        // No ID, create the first registered editor
        if ( !editorModeID.IsValid() )
        {
            KRG_ASSERT( StandaloneEditorFactory::s_pHead != nullptr );
            return StandaloneEditorFactory::s_pHead->CreateEditor();
        }

        // Else search through registered editor modes
        StandaloneEditorFactory* pEditorModeFactory = StandaloneEditorFactory::s_pHead;
        while ( pEditorModeFactory != nullptr )
        {
            ...
            pEditorModeFactory = pEditorModeFactory->GetNextItem();
        }
