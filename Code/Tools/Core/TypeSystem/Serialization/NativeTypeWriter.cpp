...
namespace KRG::TypeSystem
{
    ...
    void NativeTypeWriter::SerializeType( TypeID typeID, void const* pTypeData )
    {
        ...
        {
            TypeValueConverter::ConvertValueToString( typeID, pTypeData, m_scratchBuffer );
        ...
        else // User types
        {
            ...
            for ( auto const& entry : pTypeInfo->m_properties )
            {
                ...
                {
                    TypeValueConverter::ConvertValueToString( propInfo.m_typeID, pPropertyDataAddress, m_scratchBuffer );
                    m_writer.String( m_scratchBuffer.c_str() );
                }
                else
                {
