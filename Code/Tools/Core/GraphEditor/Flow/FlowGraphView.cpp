...
namespace KRG::GraphEditor
{
    ...
    void FlowGraphView::SetGraphToView( FlowGraph* pGraph, bool tryMaintainSelection )
    {
        if ( m_pGraph == pGraph )
        {
            return;
    ...
    void FlowGraphView::UpdateAndDraw( float childHeightOverride, void* pUserContext )
    {
    ...
    void FlowGraphView::HandleInput( DrawContext const& ctx )
    {
        // Mouse
        //-------------------------------------------------------------------------
    ...
    void FlowGraphView::OnDragNode( DrawContext const& ctx )
    {
        ...
        for ( auto const& selectedNode : m_selectedNodes )
        {
            auto pFlowNode = static_cast<Flow::Node*>( selectedNode.m_pNode );
