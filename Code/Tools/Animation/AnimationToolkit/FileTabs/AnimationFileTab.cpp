...
{
    AnimationFileTab::AnimationFileTab( EditorModel* pModel, FileSystem::Path const& sourceDataDirectory, Resource::ResourceSystem* pResourceSystem, ResourceID const& resourceID )
    ...
    void AnimationFileTab::DrawTools( UpdateContext const& context, Render::ViewportManager& viewportManager )
    {
        ...
        ImGui::BeginChild( "AnimInfo", ImVec2( 0, 18 ), false, ImGuiWindowFlags_AlwaysUseWindowPadding );
        {
            ...
            ImGui::Text( "Frame: %06.2f / %d", m_pTrackEditor->GetPlayheadPositionAsPercentage().ToFloat() * m_pResource->GetNumFrames(), m_pResource->GetNumFrames() );

            ImGuiX::VerticalSeparator();

            ImGui::Text( "Time: %05.2fs / %05.2fs", m_pTrackEditor->GetPlayheadPositionAsPercentage().ToFloat() * m_pResource->GetDuration(), m_pResource->GetDuration().ToFloat() );
        ...
        if ( ImGui::BeginTable( "LayoutTable", 2, ImGuiTableFlags_Resizable | ImGuiTableFlags_Borders | ImGuiTableFlags_SizingFixedFit ) )
        {
            ...
            auto const& selectedItems = m_pTrackEditor->GetSelectedItems();
