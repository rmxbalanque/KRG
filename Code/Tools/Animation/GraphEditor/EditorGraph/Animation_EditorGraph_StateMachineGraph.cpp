...
namespace KRG::Animation::Graph
{
    ...
    void StateEditorNode::Initialize( VisualGraph::BaseGraph* pParent )
    {
        ...
        pBlendTree->CreateNode<ResultEditorNode>( ValueType::Pose );
...
namespace KRG::Animation::Graph
{
    ...
    void EntryStateOverrideEditorGraph::Initialize( VisualGraph::BaseNode* pParentNode )
    {
        ...
        CreateNode<EntryStateOverrideConditionsEditorNode>();
    }

    void EntryStateOverrideEditorGraph::OnShowGraph()
    {
        auto conditionNodes = FindAllNodesOfType<EntryStateOverrideConditionsEditorNode>();
...
namespace KRG::Animation::Graph
{
    ...
    void GlobalTransitionsEditorGraph::UpdateGlobalTransitionNodes()
    {
        ...
        auto stateNodes = pParentSM->FindAllNodesOfType<StateBaseEditorNode>( VisualGraph::SearchMode::Localized, VisualGraph::SearchTypeMatch::Derived );

        //-------------------------------------------------------------------------

        auto globalTransitions = FindAllNodesOfType<GlobalTransitionEditorNode>();

        //-------------------------------------------------------------------------

        TInlineVector<StateBaseEditorNode*, 20> transitionsToCreate;
        TInlineVector<GlobalTransitionEditorNode*, 20> transitionsToRemove;
        ...
        auto SearchPredicate = [] ( GlobalTransitionEditorNode* pNode, UUID const& ID )
        {
            if ( pNode != nullptr )
            {
        ...
        {
            auto pNewTransition = CreateNode<GlobalTransitionEditorNode>();
...
namespace KRG::Animation::Graph
{
    ...
    EditorGraphNode const* StateMachineGraph::GetEntryConditionNodeForState( UUID const& stateID ) const
    {
        auto conditionNodes = m_pEntryOverridesConduit->GetSecondaryGraph()->FindAllNodesOfType<EntryStateOverrideConditionsEditorNode>();
        KRG_ASSERT( conditionNodes.size() == 1 );
        auto pConditionNode = conditionNodes.back();
    ...
    NodeIndex StateMachineEditorNode::Compile( EditorGraphCompilationContext& context ) const
    {
        ...
        auto const globalTransitionNodes = pStateMachine->m_pGlobalTransitionConduit->GetSecondaryGraph()->FindAllNodesOfType<GlobalTransitionEditorNode>();
        ...
        for ( auto i = 0; i < numStateNodes; i++ )
        {
            ...
            TInlineVector<GlobalTransitionEditorNode*, 20> globalTransitionNodesCopy = globalTransitionNodes;
    ...
    NodeIndex StateMachineEditorNode::CompileState( EditorGraphCompilationContext& context, StateBaseEditorNode const* pBaseStateNode ) const
    {
    ...
    NodeIndex StateMachineEditorNode::CompileTransition( EditorGraphCompilationContext& context, TransitionEditorNode const* pTransitionNode, NodeIndex targetStateNodeIdx ) const
    {
