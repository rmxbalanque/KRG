...
{
    GraphVariationEditor::GraphVariationEditor( EditorModel* pModel )
        : TEditorTool<GraphEditorModel>( pModel )
    {}

    void GraphVariationEditor::FrameStartUpdate( UpdateContext const& context, Render::ViewportManager& viewportManager )
    {
        if ( !IsOpen() )
        {
            return;
