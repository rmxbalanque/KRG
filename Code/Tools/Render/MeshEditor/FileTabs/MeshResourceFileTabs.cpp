#include "MeshResourceFileTabs.h"
...
namespace KRG::Render
{
    void StaticMeshFileTab::Activate( EntityWorld* pPreviewWorld )
    {
    ...
    void StaticMeshFileTab::Deactivate( EntityWorld* pPreviewWorld )
    {
        KRG_ASSERT( m_pPreviewEntity != nullptr );
    ...
    void StaticMeshFileTab::DrawTools( UpdateContext const& context, Render::ViewportManager& viewportManager )
    {
...
namespace KRG::Render
{
    ...
    SkeletalMeshFileTab::~SkeletalMeshFileTab()
    {
        DestroySkeletonTree();
        KRG_ASSERT( m_pSkeletonTreeRoot == nullptr );
    }

    void SkeletalMeshFileTab::Activate( EntityWorld* pPreviewWorld )
    {
        // We dont own the entity as soon as we add it to the map
        auto pPersistentMap = pPreviewWorld->GetPersistentMap();
    ...
    void SkeletalMeshFileTab::Deactivate( EntityWorld* pPreviewWorld )
    {
        auto pPersistentMap = pPreviewWorld->GetPersistentMap();
        pPersistentMap->DestroyEntity( m_pPreviewEntity->GetID() );
        m_pPreviewEntity = nullptr;
    ...
    void SkeletalMeshFileTab::DrawTools( UpdateContext const& context, Render::ViewportManager& viewportManager )
    {
        if ( IsLoading() || IsUnloaded() )
        {
            ImGui::Text( "Loading:" );
    ...
    void SkeletalMeshFileTab::CreateSkeletonTree()
    {
    ...
    void SkeletalMeshFileTab::DestroySkeletonTree()
    {
        if ( m_pSkeletonTreeRoot != nullptr )
        {
    ...
    ImRect SkeletalMeshFileTab::RenderSkeletonTree( BoneInfo* pBone ) const
    {
