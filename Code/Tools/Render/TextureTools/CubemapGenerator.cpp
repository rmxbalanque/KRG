...
namespace
{
    ...
    void SearchForFiles( const wchar_t* path, std::list<SConversion>& files, bool recursive )
    {
        ...
        std::wstring wcFilename( 255, 0 );
        mbstowcs( wcFilename.data(), findData.cFileName, 255 );
        ...
        if ( hFile )
        {
            for ( ;;)
            {
                if ( !( findData.dwFileAttributes & ( FILE_ATTRIBUTE_HIDDEN | FILE_ATTRIBUTE_SYSTEM | FILE_ATTRIBUTE_DIRECTORY ) ) )
                {
                    ...
                    SConversion conv;
                    _wmakepath_s( conv.szSrc, drive, dir, wcFilename.data(), nullptr );
...
int texassemble_main( _In_ int argc, _In_z_count_( argc ) wchar_t* argv[], Blob& ddsFile )
{
    ...
    switch ( dwCommand )
    {
        ...
        case CMD_V_STRIP:
        {
            ...
            {
                case CODEC_DDS:
                hr = SaveToDDSMemory( *dest, DDS_FLAGS_NONE, ddsFile );
                break;
        ...
        default:
        {
            ...
            hr = SaveToDDSMemory( result.GetImages(), result.GetImageCount(), result.GetMetadata(),
...
namespace KRG::Render
{
    ...
    {
        // Get wchar version of filenames
        std::wstring wcFilenames[6];
        for ( auto i = 0; i < 6; i++ )
        {
            auto len = strlen( texturePaths[i].c_str() ) + 1;
            wcFilenames[i].resize( len );
            mbstowcs( wcFilenames[i].data(), texturePaths[i].c_str(), len );
            wcFilenames[i].back() = 0;
        }
        ...
        Blob textureBlob;
        if ( texassemble_main( 13, args, textureBlob ) == 0 )
        {
            // Error occurred
