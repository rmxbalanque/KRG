...
namespace KRG::Render::Tools
{
                ...
                for ( auto const& mesh : m_assetInfo.m_meshes )
                {
                    ...
                    if ( ImGuiX::ButtonColored( buttonColor, KRG_ICON_MALE "##CreateSkeletalMesh", ImVec2( 20, 0 ) ) )
                    {
                        if ( canCreateSkeletalMesh )
                        {
                            ...
                            resourceDesc.m_meshDataPath = DataPath::FromFileSystemPath( m_pModel->GetSourceDataDirectory(), m_inspectedFile );
    ...
    DataBrowser::DataBrowser( EditorModel* pModel )
        ...
        , m_dataFileInspector( pModel )
    {
        m_onDoubleClickEventID = m_dataBrowserTreeView.OnItemDoubleClicked().Bind( [this] ( Editor::TreeViewItem* pItem ) { OnBrowserItemDoubleClicked( pItem ); } );
    ...
    {
        auto VisibilityFunc = [this] ( Editor::TreeViewItem const* pItem )
        {
            bool isVisible = true;
            ...
            auto pDataFileItem = static_cast<Editor::DataBrowserTreeItem const*>( pItem );
    ...
    void DataBrowser::DrawBrowser( UpdateContext const& context )
    {
        ...
        if ( ImGui::Begin( "Data Browser", nullptr ) )
        {
            ...
            if ( ImGui::InputText( "##Filter", m_filterBuffer, 256 ) )
            {
                ...
                auto const SetExpansion = []( Editor::TreeViewItem* pItem )
                {
                    if ( pItem->IsVisible() )
                    {
            ...
            {
                m_dataBrowserTreeView.ForEachItem( [] ( Editor::TreeViewItem* pItem ) { pItem->SetExpanded( true ); } );
            }

            ImGui::SameLine( ImGui::GetWindowContentRegionWidth() + ImGui::GetStyle().WindowPadding.x - 20 );
            if ( ImGui::Button( KRG_ICON_MINUS "##Collapse ALL" ) )
            {
                m_dataBrowserTreeView.ForEachItem( [] ( Editor::TreeViewItem* pItem ) { pItem->SetExpanded( false ); } );
            }
    ...
    void DataBrowser::OnBrowserItemDoubleClicked( Editor::TreeViewItem* pItem )
    {
        auto pDataFileItem = static_cast<Editor::DataBrowserTreeItem const*>( pItem );
