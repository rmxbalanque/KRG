...
namespace KRG
{
    ...
    class KRG_TOOLS_EDITOR_API EditorModel
    {
    ...
    public:
        ...
        void OpenResourceFile( ResourceID const& resourceID )
        {
            ...
            {
                auto pCreatedFileTab = m_openTabs.emplace_back( KRG::New<T>( this, m_sourceDataDirectory, m_pResourceSystem, resourceID ) );
                SetActiveFile( pCreatedFileTab );
            }
            else
            {
        ...
        inline bool IsActiveFile( EditorFileTab const* pFile ) const { return m_pActiveTab == pFile; }
