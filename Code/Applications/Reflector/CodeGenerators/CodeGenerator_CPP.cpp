...
namespace KRG
{
    namespace CPP
    {
        ...
        static bool GenerateTypeRegistrationHeaderFiles( ReflectionDatabase const& database, SolutionDesc const& solution, std::stringstream& typeRegistrationStr, TypeRegistrationHeaderType headerType )
        {
        ...
        void Generator::GenerateModuleCodeFile( ProjectDesc const& prj, TVector<TypeDescriptor> const& typesInModule )
        {
            ...
            char const* const extensions[] = { ".cpp", 0 };
            FileSystem::GetDirectoryContents( autoGeneratedDirectory, autoGeneratedFiles, FileSystem::DirectoryReaderOutput::OnlyFiles, FileSystem::DirectoryReaderMode::DontExpand, extensions );
        ...
        bool Generator::Generate( ReflectionDatabase const& database, SolutionDesc const& solution )
        {
            ...
            for ( auto& prj : solution.m_projects )
            {
                ...
                {
                    Printf( headerIDStringBuffer, 32, "%u", (uint32) hdrDesc.m_ID );
                    expectedFiles.push_back( FileSystem::Path( autoGeneratedDirectory + headerIDStringBuffer + ".h" ) );
                    expectedFiles.push_back( FileSystem::Path( autoGeneratedDirectory + headerIDStringBuffer + ".cpp" ) );
                }
                ...
                for ( auto& dirtyHeaderIdx : prj.m_dirtyHeaders )
                {
                    ...
                    Printf( headerIDStringBuffer, 32, "%u", (uint32) hdr.m_ID );
