...
namespace KRG
{
    namespace TypeSystem
    {
        namespace Reflection
        {
            ...
            {
                if ( !slnPath.IsValid() || slnPath.IsDirectoryPath() || !slnPath.MatchesExtension( "sln" ) || !slnPath.Exists() )
                {
                ...
                std::cout << std::endl << " * Solution Parsing - Complete! ( " << (float) m_slnParsingTime << " ms )" << std::endl;
                return true;
            }
                ...
                Milliseconds time = 0;
                {
                    ...
                    for ( auto& prj : m_solution.m_projects )
                    {
                        ...
                        for ( auto i = 0u; i < prj.m_headerFiles.size(); i++ )
                        {
                            ...
                            Printf( targetFilename, 32, "%u", (uint32) header.m_ID );
                            FileSystem::Path const autoGeneratedTargetHeaderFilePath( autoGeneratedDirectory + targetFilename + ".h" );
                            FileSystem::Path const autoGeneratedTargetCodeFilePath( autoGeneratedDirectory + targetFilename + ".cpp" );
            ...
            bool Reflector::ParseRegisteredHeaders()
            {
                ...
                std::cout << "Complete! ( P:" << (float) m_clangParsingTime << "ms, V:" << (float) m_clangVisitingTime << "ms )" << std::endl;
                return true;
            }
