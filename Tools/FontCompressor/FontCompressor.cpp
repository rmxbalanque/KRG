...
int main(int argc, char** argv)
{
    ...
    int argn = 1;
    binary_to_compressed_c(argv[argn], argv[argn+1]);
    return 1;
}
...
bool binary_to_compressed_c(const char* filename, const char* symbol)
{
    ...
    const char* compressed_str = "compressed_";

    // Max blob size = 50kb
    int const blob_max_size = 50 * 1024;
    int num_blobs = ( compressed_sz / blob_max_size ) + 1;
    int byte_range_start = 0;
    for ( int blob_number = 0; blob_number < num_blobs; blob_number++ )
    {
        int byte_range_end = MIN( ( blob_number + 1 ) * blob_max_size, compressed_sz );

        fprintf( out, "static const char %s_%sdata_base85_%d[] =\n    \"", symbol, compressed_str, blob_number );
        char prev_c = 0;
        for ( int src_i = byte_range_start; src_i < byte_range_end; src_i += 4 )
        {
